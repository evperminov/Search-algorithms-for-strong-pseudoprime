...
{
  factors->p = (__mpz_struct (*)[1]) malloc (1);
  factors->e = (long unsigned int*) malloc (1);
  factors->nfactors = 0;
}
...
// void ~factors (struct factors *factors)
// {
//   int i;
...
factor_insert (struct factors *factors, mpz_t prime)
{
  ...
  if (i < 0 || mpz_cmp (p[i], prime) != 0)
    {
      p = (__mpz_struct (*)[1])realloc (p, (nfactors + 1) * sizeof p[0]);
      e = (long unsigned int*)realloc (e, (nfactors + 1) * sizeof e[0]);
...
void factor (mpz_t t, struct factors *factors)
{
  factor_init (factors);
