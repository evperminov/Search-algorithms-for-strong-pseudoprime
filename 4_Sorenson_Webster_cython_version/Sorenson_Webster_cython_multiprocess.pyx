...
# Менеджер для общих ресурсов процессов
...
cdef vector[vector[uint8_t]] sign_keys
...
cdef void create_coeff_wheel(vector[uint8_t]& residue, uint8_t module):
    ...
    ## Определение коэффуциентов и сохранение
...
cdef void find_h(mpz_t* h_z, vector[uint8_t]& sign, mpz_t k):
    ### Инициализация
    global v_z, v
    cdef mpz_t u_z, k_1_z, h_i_z
    mpz_inits(u_z, k_1_z, h_i_z, NULL)
    ...
    for i in range(v.size()):
        if sign[i] > 0:
            mpz_pow_ui(h_i_z, v_z[i], u * pow(2, sign[i] - 1))
    ...
    ### Очистка памяти
    mpz_clears(u_z, k_1_z, h_i_z, NULL)


# Поиск pt методом gcd
cdef void search_pt_GCD(vector[uint8_t]& sign, mpz_t pt_1_z, mpz_t k_z, cur_spsp, lock_spsp): 
    ### Инициализация 
    global v, g_z
...
cdef void sieve(mpz_t k_z, mpz_t pt_1_z, mpz_t r_z, mpz_t mod_z, cur_spsp, lock_spsp):
    ### Инициализация
...
# Функция для просеивания подходящих чисел
# по заданному классу остатков
cdef void sieve_single_proc(mpz_t k_z, mpz_t pt_1_z, mpz_t r_z, mpz_t mod_z):
    ### Инициализация
    global g_z, cur_spsp_single_proc
    cdef mpz_t low_border_z, up_border_z, tmp_z
    cdef mpz_t step_z, pt_z, output
    mpz_inits(tmp_z, pt_z, output, NULL)
    ...
    while mpz_cmp(up_border_z, step_z) > 0:
        ...
        if mpz_probab_prime_p(pt_z, 15) > 0:
            mpz_mul(output, k_z, pt_z)
            if miller_rabin(output):
                cur_spsp_single_proc.append(int(GMPy_MPZ_From_mpz(output)))
        mpz_add_ui(step_z, step_z, 1)
...
cdef void search_pt(bint use_leg_tup, uint8_t coeff, mpz_t r_d_z, mpz_t modd_d_z, mpz_t r_l_z, mpz_t lambd_z, mpz_t pt_1_z, mpz_t k_z, cfactors w_cf, cur_spsp, lock_spsp):
    ### Инициализация
    global leg_tup_one
    ...
    for i in range(cur_residues.size()): 
        ...
        sieve(k_z, pt_1_z, cur_r_z, cur_m_z, cur_spsp, lock_spsp)
...
cdef void tup_check_t_more_3(mpz_t k_z, vector[vector[uint32_t]] & tup, vector[uint8_t] & sign):
    ...
    sieve_single_proc(k_z, pt_1_z, c_z, lambda_z)
...
cdef void tup_check_t_equal_3(mpz_t k_z, vector[vector[uint32_t]] & tup, vector[uint8_t] & sign, cur_spsp, lock_spsp):
    ### Инициализация
    global X_z
    cdef mpz_t lambda_z, tmp_z, pt_1_z, c_z
    mpz_init_set_ui(lambda_z, 1)
    ...
    if mpz_cmp(k_z, X_z) <= 0:
        search_pt_GCD(sign, pt_1_z, k_z, cur_spsp, lock_spsp)
    else:
        mpz_invert(c_z, k_z, lambda_z)
        lambda_signature_sieving(pt_1_z, c_z, lambda_z, k_z, cur_spsp, lock_spsp)
...
# Рекурсивная функция для генерации подходящих кортежей для t=3
cdef uint32_t rec_comp_feas_tup_t3(vector[uint8_t] & sign, vector[vector[uint32_t]] & primes, uint8_t deep, uint32_t up_border, vector[vector[uint32_t]] & tup, mpz_t k_z, uint32_t low_border, cur_spsp, lock_spsp):
    ### Инициализация
    global g_z, tup_check
    cdef uint32_t i = low_border
    cdef mpz_t cur_k_z, cur_k_check_z
    ...
    if deep !=0:
        while i < up_border:
            tup[deep] = primes[i]
            mpz_mul_ui(cur_k_z, k_z, primes[i][0])
            if rec_comp_feas_tup_t3(sign, primes, deep - 1, up_border, tup, cur_k_z, i + 1, cur_spsp, lock_spsp) == 0:
                break
            i += 1
    else:
        ### Перебор на позицию i = t - 1 
        while i < up_border:
            ...
            tup[deep] = primes[i]
            tup_check_t_equal_3(cur_k_z, tup, sign, cur_spsp, lock_spsp)
            i = i + 1
...
# Рекурсивная функция для генерации подходящих кортежей для t=3
# с заранее известным значением pt-1
cdef uint32_t rec_comp_feas_tup_t3_with_known_p_t_1(vector[uint8_t] & sign, vector[vector[uint32_t]] & primes, uint8_t deep, uint32_t up_border, vector[vector[uint32_t]] & tup, mpz_t k_z, uint32_t low_border, cur_spsp, lock_spsp):
    ### Инициализация 
    global g_z, tup_check
    cdef uint32_t i = low_border
    cdef mpz_t cur_k_z, cur_k_check_z
    ...
    if deep != 1:
        while i < up_border:
            tup[deep] = primes[i]
            mpz_mul_ui(cur_k_z, k_z, primes[i][0])
            if rec_comp_feas_tup_t3_with_known_p_t_1(sign, primes, deep - 1, up_border, tup, cur_k_z, i + 1, cur_spsp, lock_spsp) == 0:
                break
            i += 1
    else:
        ### Перебор на позицию i = t - 2
...
cdef void lambda_signature_sieving(mpz_t pt_1_z, mpz_t r_l_z, mpz_t lambd_z, mpz_t k_z, cur_spsp, lock_spsp):
    ## Инициализация
    global v_z
    cdef uint64_t e, f
    ...
    if w_cf.nfactors == 0:
        sieve(k_z, pt_1_z, r_l_z, lambd_z, cur_spsp, lock_spsp)
    ...
    if mpz_cmp_ui(temp_z, 3) == 0:
        ...
        search_pt(True, 3, r_d_z, modd_d_z, r_l_z, lambd_z, pt_1_z, k_z, w_cf, cur_spsp, lock_spsp)
        # p1 = 3 mod 4 and p2 = 1 mod 4 (bin sign)
        search_pt(False, 1, r_d_z, modd_d_z, r_l_z, lambd_z, pt_1_z, k_z, w_cf, cur_spsp, lock_spsp) 
    ...
    if mpz_cmp_ui(temp_z, 5) == 0:
        ...
        search_pt(True, 1, r_d_z, modd_d_z, r_l_z, lambd_z, pt_1_z, k_z, w_cf, cur_spsp, lock_spsp)  
...
# Функция для поиска spsp с количеством простых множителей = 3
cdef void find_spsp_with_three_factors(indx, lock_it, uint8_t n_proc, uint8_t id_proc, uint64_t step_size, cur_spsp, lock_spsp):
    ### Инициализация 
    global sorted_primes_by_sign, v, g_z, X, sign_keys
    cdef mpz_t p_z, k_z, border_up_z, lambda_z
    mpz_inits(p_z, k_z, border_up_z, lambda_z, NULL)
    ...
    cdef vector[uint8_t] sign
    cdef vector[vector[uint32_t]] primes
    cdef uint64_t border_up, cur_start, cur_stop, p, sign_idx

    mpz_fdiv_q_ui(border_up_z, g_z, n_primes(1, v[v.size() - 1] + 2)[0])
    mpz_sqrt(border_up_z, border_up_z)
    border_up = mpz_get_ui(border_up_z)
    ...
    lock_it.release()
    while sign_idx < sign_keys.size():
        keyss = sign_keys[sign_idx]
        primes = sorted_primes_by_sign[keyss]
        if primes.size() >= 2:
            mpz_set_ui(k_z, 1)
            rec_comp_feas_tup_t3(keyss, primes, 1, primes.size(), tup, k_z, 0, cur_spsp, lock_spsp)
        lock_it.acquire()
        sign_idx = indx.value 
        indx.value = indx.value  + 1
        lock_it.release()
    ...
    ## Составление подходящих кортежей на основе 
    ## предварительно сохраненных сигнатур и только что найденных  
    cur_start = id_proc * step_size + X
    while cur_start < border_up:
        cur_stop = cur_start + step_size
        if cur_stop > border_up:
            cur_stop = border_up
        it.skipto(cur_start)
        p = it.next_prime()
        mpz_set_ui(p_z, p)
        while p < cur_stop:
            ...
            if primes.size() >= 1:
                ...
                tup[0][0] = p
                tup[0][1] = mpz_get_ui(lambda_z)
                rec_comp_feas_tup_t3_with_known_p_t_1(sign, primes, 1, primes.size(), tup, p_z, 0, cur_spsp, lock_spsp)
            p = it.next_prime()
            mpz_set_ui(p_z, p)  
        cur_start = cur_start +  n_proc * step_size
...
cdef void find_spsp_with_two_factors(indx, lock_it, uint8_t n_proc, uint8_t id_proc, uint64_t step_size,  cur_spsp, lock_spsp, uint64_t up_border_p1):
    ### Инициализация
    global sorted_primes_by_sign, X, sign_keys
    cdef vector[vector[uint32_t]] primes
    cdef vector[uint8_t] keyss
    cdef mpz_t p_z, r_z, lambda_z
    mpz_init_set_ui(r_z, 1)
    mpz_inits(p_z, lambda_z, NULL)
    cdef uint64_t p, sign_idx
    it = Iterator()
    cdef uint64_t cur_start = id_proc * step_size + X
    cdef uint64_t cur_stop
    ...
    while sign_idx < sign_keys.size():
        ...
        for i in range(primes.size()):
            mpz_set_ui(p_z, primes[i][0])
            search_pt_GCD(keyss, p_z, p_z, cur_spsp, lock_spsp)
        lock_it.acquire()
        sign_idx = indx.value 
        indx.value = indx.value  + 1
        lock_it.release()
    ...
    while cur_start < up_border_p1:
        cur_stop = cur_start + step_size
        if cur_stop > up_border_p1:
            cur_stop = up_border_p1
        it.skipto(cur_start)
        p = it.next_prime()
        mpz_set_ui(p_z, p)
        while p < cur_stop:
            find_lambda(lambda_z, p_z)
            lambda_signature_sieving(p_z, r_z, lambda_z, p_z, cur_spsp, lock_spsp)
            p = it.next_prime()
            mpz_set_ui(p_z, p)
        cur_start = cur_start +  n_proc * step_size
...
def sorenson(mpz g, uint8_t v_count, uint64_t coeff):
    ...
    global v_z, v, g_z, X, w_border_z, X_z, sorted_primes_by_sign, cur_spsp_single_proc
    global leg_tup_one, sign_keys
    ...
    # Распараллеливание
    CustomManager.register('Iterator', Iterator)
    manager = Manager()
    lock_it = Lock()
    lock_spsp = Lock()
    cdef uint8_t n_proc = 4
    cdef mpz_t step_size_z 
    mpz_init(step_size_z)
    mpz_root(step_size_z, g_z, 4)
    cdef uint64_t step_size = mpz_get_ui(step_size_z)

    # Предварительное вычисление сигнатур
    start_time = time.perf_counter()
    get_signatures() 
    for sign in sorted_primes_by_sign:
        sign_keys.push_back(sign.first)
    logging.info(f'get_signatures : {time.perf_counter() - start_time:.4f} seconds')

    # Запуск поиска spsp для t > 3
    for t in range(6, 3, -1):
        cur_spsp_single_proc = spsp[t]
        start_time = time.perf_counter()
        find_spsp_with_more_than_three_factors(t)
        logging.info(f'spsp {t} : {time.perf_counter() - start_time:.4f} seconds')

    # Распараллеливание для случая t=3 и t=2
    with CustomManager() as cust_manager:
        processes = []
        cur_spsp = manager.list()
        indx = manager.Value('i', 0)   

        # Запуск поиска spsp для t = 3
        start_time = time.perf_counter()
        for id_proc in range(n_proc):
            proc = Process(target=find_spsp_with_three_factors, args=(indx, lock_it, n_proc, id_proc, step_size, cur_spsp, lock_spsp,))
        ...
        start_time = time.perf_counter()
        for id_proc in range(n_proc):
            proc = Process(target=find_spsp_with_two_factors, args=(indx, lock_it, n_proc, id_proc, step_size, cur_spsp, lock_spsp, up_border_p1,))
            processes.append(proc)
            proc.start()
    ...
    for i in range(v.size()):
        mpz_clear(v_z[i])
    free(v_z)
    mpz_clears(g_z, X_z, up_border_p1_z, w_border_z, step_size_z, NULL)
